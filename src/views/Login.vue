...
<template>
    <div class="size-full flex-center">
        <form class="flex space-x-4 w-full justify-center">
            ...
            <div class="flex flex-col mb-6 transition-[opacity,transform] duration-500"
                ...
                <input
                    class="shadow-sm appearance-none border rounded-sm w-full py-2 px-3 text-gray-700 mb-3 leading-tight focus:outline-hidden focus:shadow-outline"
                    id="password" type="password" v-model="password">
            </div>
            <div class="flex items-center transition-[opacity,transform] duration-500"
                :class="email.length > 0 && password.length > 0 ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-full'">
                <button
                    class="bg-blue-500 hover:bg-blue-700 disabled:bg-blue-300 text-white font-bold py-2 px-4 rounded-sm focus:outline-2"
